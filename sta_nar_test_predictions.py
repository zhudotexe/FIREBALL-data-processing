...
MODEL_NAME = "dialog-continuation"  # CHANGE ME
...
def main():
    ...
    for datum in test_data:
        # prompt = prompts.sta_nar_prompt(datum, ablations=["actors", "targets", "caster"])  # CHANGE ME FOR ABLATIONS
        prompt = prompts.sta_nar_dialog_continuation_prompt(datum)
        try:
            prediction = get_prediction(prompt, model=DIALOG_CONTINUATION_MODEL)  # CHANGE ME FOR ABLATIONS TOO
